...
postprocess:
	...
	udapy read.Conllu files=data/annotated/13_19_stol/$(ANNBASE)_$(A1).conllu ud.FixAdvmodByUpos ud.FixMultiSubjects util.Eval node='if node.upos=="PUNCT": node.deprel="punct"' ud.FixPunct ud.FixLeaf ud.FixRightheaded deprels=conj,flat,fixed,appos,goeswith,list write.Conllu files=data/annotated/13_19_stol/$(ANNBASE)_$(A1).fixed.conllu
	udapy read.Conllu files=data/annotated/13_19_stol/$(ANNBASE)_$(A2).conllu ud.FixAdvmodByUpos ud.FixMultiSubjects util.Eval node='if node.upos=="PUNCT": node.deprel="punct"' ud.FixPunct ud.FixLeaf ud.FixRightheaded deprels=conj,flat,fixed,appos,goeswith,list write.Conllu files=data/annotated/13_19_stol/$(ANNBASE)_$(A2).fixed.conllu
